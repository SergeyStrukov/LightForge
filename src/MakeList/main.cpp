...
class FileList
 {
   std::vector<File> cppList;
   std::vector<File> asmList;
  ...
  private:
   ...
   void process(std::vector<File> &list);
  ...
  if( name.extension()==".cpp" )
    {
     ...
     cppList.emplace_back(path,name,name.stem());
    }
  ...
  if( name.extension()==".s" )
    {
     ...
     asmList.emplace_back(path,name,name.stem());
    }
 }
...
void FileList::process(std::vector<File> &list)
 {
  auto len=list.size();
...
FileList::FileList()
 {
  cppList.reserve(BigReserve);
  asmList.reserve(BigReserve);
 }
...
void FileList::print(std::ostream &out) const
 {
  ...
  for(const File &file : cppList )
    {
     out << "$(OBJ_PATH)/" << file.getNoExt() << ".o \\\n" ;
    }

  for(const File &file : asmList )
    {
     out << "$(OBJ_PATH)/" << file.getName() << ".o \\\n" ;
    }
  ...
  for(const File &file : asmList )
    {
     ...
     out << "$(OBJ_PATH)/" << file.getName() << ".o : " << file.getPath() << "\n" ;
     out << "\t$(AS) $(ASOPT) $< -o $@\n\n" ;
    }
