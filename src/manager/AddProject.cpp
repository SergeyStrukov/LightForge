...
static void CreateProjectMakefile(const Path &folder,const Path &projRoot)
 {
...
static void CreateTargetMakefile(const Path &dir,const TargetInfo &target)
 {
  ...
  out << "TARGET_PATH = " << (const String &)target.path << "\n\n" ;
  ...
  out << "INCDIR_LIST =" << PrintList(target.inc) << PrintList(target.incPrivate) << "\n\n" ;
...
static void AddTarget(const Path &folder,const TargetInfo &target)
 {
...
static void AddProject(const Path &forge,const char *build,const String &projName,Path projRoot,const std::vector<String> &baseList,const std::vector<TargetReader> &targets)
 {
  ...
  for(const TargetReader &target : targets )
    {
     AddTarget(folder,target.getInfo());
    }
 }
